(function ($) {
    ...
    dropdown.buildCartItem = function ($elem, data) {
        ...
        for (var i = 0; i < data.length; i++) {
            html += '<li class="cart-item"><a href="###" target="_blank" class="cart-item-pic fl"><img src="' + data[i].image + '" alt="" /></a><div class="fl"><p class="cart-item-name text-ellipsis"><a href="###" target="_blank" class="link">' + data[i].name + '</a></p><p class="cart-item-price"><strong>￥' + data[i].price + ' x ' + data[i].num + '</strong></p></div><a href="javascript:;" title="删除" class="cart-item-delete link fr">X</a></li>';
        }
        ...
        // setTimeout(function () {
            $elem.find('.dropdown-layer').html(html);
            dropdown.updataCart($elem, data);
        // }, 1000);
    }

    dropdown.updataCart = function ($elem, data) {
    ...
    slider.$focusSlider.on('focus-loadItem',function(e,index,elem,success){
        imgLoader.loadImgs($(elem).find('.slider-img'),success,function($img,url){
            $img.attr('src','image/focus-slider/placeholder.png');
        });
    });
    lazyLoad.loadUntil({
    ...
    slider.$todaysSlider.on('todays-loadItem',function(e,index,elem,success){
        imgLoader.loadImgs($(elem).find('.slider-img'),success,function($img,url){
            $img.attr('src','image/todays-slider/placeholder.png');
        });
    });
    lazyLoad.loadUntil({
    ...
    //                 $elem.loadedItemNum++;
    //                 console.log($elem.attr('id') + "-" + index + ":loaded");
    ...
    floor.buildFloorBody=function (floorData) {
        ...
        for (var i = 0; i < floorData.items.length; i++) {
            ...
            for (var j = 0; j < floorData.items[i].length; j++) {
                ...
                html += '<p class="floor-item-pic"><a href="###" target="_blank"><img src="image/floor/loading.gif" class="floor-img" data-src="image/floor/' + floorData.num + '/' + (i + 1) + '/' + (j + 1) + '.png" alt="" /></a></p>';
    ...
    floor.whichFloor=function(){
        ...
        floor.$floor.each(function(index,elem){
            ...
            num=index;
            if(floor.$win.scrollTop()+floorData.height/2<floorData.offsetTop){
                num=index-1;
                return false;
            }
        });
    ...
    foot.timeToShow=function(){
        if(lazyLoad.isVisible(foot.$helper.offsetTop,foot.$helper.height)||lazyLoad.isVisible(foot.$helper.offsetTop,foot.$helper.height)){
